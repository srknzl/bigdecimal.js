...
suite.add('Bigdecimal.js', function () {
    for (let i = 0; i < bigDecimals.length - 1; i++) {
        bigDecimals[i].pow(exponents[i%length], MC(precision, RoundingMode.HALF_UP));
    }
}).add('Big.js', function () {
    ...
    for (let i = 0; i < bigDecimalsGWT.length - 1; i++) {
        bigDecimalsGWT[i].pow(exponents[i%length], new MathContext(`precision=${precision} roundingMode=HALF_UP`));
    }
});
